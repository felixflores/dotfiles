...
class Sheller
  ...
  def shell_file
    return ['zsh','~/.zshrc'] if ENV['SHELL'].include? 'zsh'
    return ['bash','~/.bashrc'] if ENV['SHELL'].include? 'bash'
    raise UnsupportedShellError
  end

  def sources(type)
    sources = %w(zsh/aliases zsh/functions) if type == 'zsh'
  ...
  def dotfile_path
    ...
    @dotfile_path = @link_to_dotfile.gsub(/#{home}/,'$HOME')
  end
  ...
  def update_shell(type, path)
    ...
    unless sources_to_add.size.zero?
